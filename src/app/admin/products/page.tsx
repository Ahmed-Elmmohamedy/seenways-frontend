...
function ColorVariantEditor({ variant, index, onChange, onRemove, onUpload }: {
  ...
  const removeImage = (imgIdx: number) => onChange({ ...variant, images: variant.images.filter((_, i) => i !== imgIdx) });
            ...
            <div className="space-y-2">
              {variant.sizes.map((s, sIdx) => (
                <div key={sIdx} className="flex items-center gap-2">
                  ...
                  <button type="button" onClick={() => removeSize(sIdx)} className="text-gray-300 hover:text-red-400 transition-colors">
                    <X size={14} />
                  </button>
                </div>
              ))}
  ...
  return (
    <div dir="rtl">
      ...
      <div className="flex gap-3 mb-5">
        ...
        <button onClick={load} className="border border-gray-200 px-5 py-2.5 text-xs tracking-widest uppercase hover:border-black transition-colors">
          بحث
        </button>
      </div>
